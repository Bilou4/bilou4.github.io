...
![hiddenEssid](./img/hiddenESSID.png)
...
The RC4 base - stream Cipher
Introduce a random element in the encrypted data.
VI-24 bit is too small to prevent repetition.
Key can be recovered in 3 min (usually).
...
![bruteforceAttack](./img/bruteforceAttack.png)

We have a dictionary of mdp (sentence between 8 and 63 char). We try to connect to the access point by generating PSKs to create PTKs and check the match with the MIC.

WPA/WPA2 are also vulnerable to attacks against AP misconfiguration (WPS and PIN)

Usually 8 numbers or 100 000 000 possibilities
...
```sh
airbase-ng -c [n°Channel] -e "AccessName" [interfaceMonitor] # create a fake access point

airbase-ng -c [n°Channel (less than default channel)] -a [BSSID] -e "ESSIDname" -L -W1 [interfaceMonitor] # copy an access point
...
airbase-ng --essid [netname=mitm] -c [n°Channel] [interfaceMonitor] # create an access point. This also creates an intermediate interface 'at0' for bridging.
